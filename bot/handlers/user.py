...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if not is_registered:
        ...
        exp_line = f"\nПодписка до: {expires}" if expires and is_pro else ""
        text = (
            f"Привет, <b>{message.from_user.first_name}</b>!\n\n"
...
async def cb_analyze(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.answer("Отправьте URL сайта:", reply_markup=cancel_kb())
    await cb.answer()
...
async def cb_lite_run(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.answer("Отправьте URL для теста:", reply_markup=cancel_kb())
    await cb.answer()
...
async def cb_pro_run(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.answer("Отправьте URL:", reply_markup=cancel_kb())
    await cb.answer()
...
async def cb_stress_flood(cb: CallbackQuery):
    ...
    if not storage.is_pro(uid):
        ...
        return
    await cb.message.answer(
        "<b>Flood-тест</b>\n\nАгрессивный режим: максимальный поток запросов, ротация заголовков и сессий, GET/POST/HEAD.\nВыберите профиль:",
        parse_mode=ParseMode.HTML,
...
async def cb_flood_run(cb: CallbackQuery, state: FSMContext):
    _, duration, intensity = cb.data.split(":")
    await state.set_state(UserStates.stress_flood_waiting_url)
    await state.update_data(mode="flood", duration=int(duration), intensity=intensity)
    await cb.message.answer("Отправьте URL:", reply_markup=cancel_kb())
    await cb.answer()
